...
if __name__ == '__main__':
    ...
    count_sql = "select count(*) from cfa_cn; "
    cur.execute(count_sql)
    long_count = cur.fetchone()['count(*)']
    # sql 语句
    big_list = []
    for num in range(1, long_count):
        sql = 'select name,city,GetYear,oneUrl  from cfa_cn where id = %s ' % num
        # #执行sql语句
        cur.execute(sql)
        # #获取所有记录列表
        data = cur.fetchone()
    ...
    head = ['name', 'city', "GetYear", "oneUrl"]
    csv_dict_write('{0}/cfa_cn.csv'.format(l_path), head, big_list)
    print("数据导出完成～")